...
##### Model without item random effect: Rule at school: Answer ~ Task + Age + (1|subid)
1.  No significant interaction between task and age ($\chi^{2}$ = 0.67, *p* = 0.41)
2.  No significant differences between tasks ($\beta$ = 0.64, *p* = .15)
3.  No significant effect of age ($\beta$ = 0.30, *p* = .64)
```{r, include = FALSE}
rule.iran.noitem <- glmer(answer ~ task*age.c + (1|subid), 
                       data = subset(acceptability.ms, q_kind == 2 & site == "Iran"), 
                       family = 'binomial') # converges
##does the interaction improve the model fit?
rule.iran.base.noitem <- glmer(answer ~ task+ age.c + (1|subid), 
                       data = subset(acceptability.ms, q_kind == 2 & site == "Iran"), 
                       family = 'binomial') # converges
anova(rule.iran.base.noitem, rule.iran.noitem, test = 'lrt') #does not improve model fit
```

```{r}
#Main effects
knitr::kable(summary(rule.iran.base.noitem)$coef)
```
...
omnibus.base <- lmer(answer ~ task + age.c + (1|subid) + (task|site) + (1|item), 
                data = ratings.ms, 
                control=lmerControl(optimizer="bobyqa",
                                             optCtrl=list(maxfun=2e4)))
omnibus.site <- lmer(answer ~ site + task + age.c + (1|subid) + (1|site), 
                data = ratings.ms, 
                control=lmerControl(optimizer="bobyqa",
                                             optCtrl=list(maxfun=2e4)))
anova(omnibus.base, omnibus.site, test = 'lrt') ## significant effect of site

##testing for lower-order interaction
omnibus.taskageint <- lmer(answer ~ site + task*age.c +  (1|subid), 
                data = ratings.ms, 
                control=lmerControl(optimizer="bobyqa",
                                             optCtrl=list(maxfun=2e4)))
anova(omnibus.site, omnibus.taskageint)

omnibus <- lmer(answer ~ site * task * age.c + (1|subid), 
                data = ratings.ms, 
                control=lmerControl(optimizer="bobyqa",
                                             optCtrl=list(maxfun=2e4)))

##testing for full 3 way
anova(omnibus.taskageint, omnibus) ## three way interaction
summary(omnibus)
anova(omnibus)

library(lsmeans)

emmeans(omnibus, pairwise~site*task, by = "age.c")
```
