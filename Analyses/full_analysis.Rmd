...
study1_data %>%
  ...
  mutate(task = factor(task, levels = c("moral", "conv"), 
                       labels = c("Moral", "Conventional")), 
         q_kind_label = factor(q_kind_label, labels = c("In a faraway country?", 
                                                        "If there's no school rule?",
...
ggsave("mean_acceptability.png", height = 3, width = 7)
```
...
### ... Is there an interaction between circumstances and task? 

```{r}
acceptability.ms.int <- acceptability.ms %>%
  mutate(q_kind = factor(q_kind))

circumstances_int_india <- glmer(answer ~ task + age.c + (1|subid), 
                                 family = binomial, 
                                 control=glmerControl(optimizer="bobyqa",
                                             optCtrl=list(maxfun=2e4)),
                                 data = subset(acceptability.ms.int, site == "India"))
circumstances_int_india_q <- glmer(answer ~ q_kind + task + age.c + (1|subid), 
                                 family = binomial, 
                                 control=glmerControl(optimizer="bobyqa",
                                             optCtrl=list(maxfun=2e4)),
                                 data = subset(acceptability.ms.int, site == "India"))
circumstances_int_india_int <- glmer(answer ~ q_kind*task + age.c + (1|subid), 
                                 family = binomial, 
                                 control=glmerControl(optimizer="bobyqa",
                                             optCtrl=list(maxfun=2e4)),
                                 data = subset(acceptability.ms.int, site == "India"))
anova(circumstances_int_india, circumstances_int_india_q, circumstances_int_india_int, 
      test = 'lrt')

summary(circumstances_int_india_int)
emmeans::emmeans(circumstances_int_india_int, pairwise~q_kind*task, adjust = "none")
```
...
### ... is there an interaction between circumstances and task in acceptability rating?

```{r}
circumstances_int_iran <- glmer(answer ~ task + age.c + (1|subid), 
                                 family = binomial, 
                                 control=glmerControl(optimizer="bobyqa",
                                             optCtrl=list(maxfun=2e4)),
                                 data = subset(acceptability.ms.int, site == "Iran"))
circumstances_int_iran_q <- glmer(answer ~ q_kind + task + age.c + (1|subid), 
                                 family = binomial, 
                                 control=glmerControl(optimizer="bobyqa",
                                             optCtrl=list(maxfun=2e4)),
                                 data = subset(acceptability.ms.int, site == "Iran"))
circumstances_int_iran_int <- glmer(answer ~ q_kind*task + age.c + (1|subid), 
                                 family = binomial, 
                                 control=glmerControl(optimizer="bobyqa",
                                             optCtrl=list(maxfun=2e4)),
                                 data = subset(acceptability.ms.int, site == "Iran"))
anova(circumstances_int_iran, circumstances_int_iran_q, circumstances_int_iran_int, 
      test = 'lrt')

summary(circumstances_int_iran_int)
emmeans::emmeans(circumstances_int_iran_int, pairwise~q_kind*task, adjust = "none")
```
...
```{r}
circumstances_int_canada <- glmer(answer ~ task + age.c + (1|subid), 
                                 family = binomial, 
                                 control=glmerControl(optimizer="bobyqa",
                                             optCtrl=list(maxfun=2e4)),
                                 data = subset(acceptability.ms.int, site == "Canada"))
circumstances_int_canada_q <- glmer(answer ~ q_kind + task + age.c + (1|subid), 
                                 family = binomial, 
                                 control=glmerControl(optimizer="bobyqa",
                                             optCtrl=list(maxfun=2e4)),
                                 data = subset(acceptability.ms.int, site == "Canada"))
circumstances_int_canada_int <- glmer(answer ~ q_kind*task + age.c + (1|subid), 
                                 family = binomial, 
                                 control=glmerControl(optimizer="bobyqa",
                                             optCtrl=list(maxfun=2e4)),
                                 data = subset(acceptability.ms.int, site == "Canada"))
anova(circumstances_int_canada, circumstances_int_canada_q, circumstances_int_canada_int, 
      test = 'lrt')

summary(circumstances_int_canada_int)
emmeans::emmeans(circumstances_int_canada_int, pairwise~q_kind*task, adjust = "none")
```
...
```{r}
circumstances_int_korea <- glmer(answer ~ task + age.c + (1|subid), 
                                 family = binomial, 
                                 control=glmerControl(optimizer="bobyqa",
                                             optCtrl=list(maxfun=2e4)),
                                 data = subset(acceptability.ms.int, site == "Korea"))
circumstances_int_korea_q <- glmer(answer ~ q_kind + task + age.c + (1|subid), 
                                 family = binomial, 
                                 control=glmerControl(optimizer="bobyqa",
                                             optCtrl=list(maxfun=2e4)),
                                 data = subset(acceptability.ms.int, site == "Korea"))
circumstances_int_korea_int <- glmer(answer ~ q_kind*task + age.c + (1|subid), 
                                 family = binomial, 
                                 control=glmerControl(optimizer="bobyqa",
                                             optCtrl=list(maxfun=2e4)),
                                 data = subset(acceptability.ms.int, site == "Korea"))
anova(circumstances_int_korea, circumstances_int_korea_q, circumstances_int_korea_int, 
      test = 'lrt')

summary(circumstances_int_korea_int)
emmeans::emmeans(circumstances_int_korea_int, pairwise~q_kind*task, adjust = "none")
```
...
---
## Comparison of acceptability in different circumstances 
```{r}
## visualization of what this looks like per site
acceptability.ms %>%
  ...
  labs(y = "Mean acceptability", 
       ...
       fill = "Situation") +
  facet_grid(~site)
ggsave("acceptability_ratings.png", height = 3)
```

### ... Do circumstances change response?


#### ...models
What we want to know here is whether the circumstances change the acceptability of a particular transgression. The approach here is to see whether, for moral and conventional items separately, circumstances meaningfully change acceptability

#### Canada

1.  Significant effect of circumstances for both moral and conventional items
2.  Moral & Conventional: Rule at school is significantly more permissible than either everyone else or faraway country

```{r}
## Moral
## singular fit with item level intercept
canada_circumstances_moral_base <- glmer(answer ~ age.c + (1|subid),
                    data = subset(acceptability.ms, 
                                  site == "Canada" & task == "moral"),
                    family = 'binomial', 
                     control=glmerControl(optimizer="bobyqa",
                                             optCtrl=list(maxfun=2e4)))
## add q_kind_label
canada_circumstances_moral <- glmer(answer ~ q_kind_label + age.c + (1|subid),
                    data = subset(acceptability.ms, 
                                  site == "Canada" & task == "moral"),
                    family = 'binomial', 
                     control=glmerControl(optimizer="bobyqa",
                                             optCtrl=list(maxfun=2e4)))
anova(canada_circumstances_moral_base, canada_circumstances_moral, test = 'lrt') # significant effect of q_kind label, chisq = 77.175

## pairwise comparisons
emmeans::emmeans(canada_circumstances_moral, pairwise~q_kind_label, adjust = "none")
## significant difference between everyone else and rule at school
## significant difference between faraway country and rule at school

## conventional
## singular fit with item level intercept
canada_circumstances_conv_base <- glmer(answer ~ age.c + (1|subid),
                    data = subset(acceptability.ms, 
                                  site == "Canada" & task == "conv"),
                    family = 'binomial', 
                     control=glmerControl(optimizer="bobyqa",
                                             optCtrl=list(maxfun=2e4)))
## add q_kind_label
canada_circumstances_conv <- glmer(answer ~ q_kind_label + age.c + (1|subid),
                    data = subset(acceptability.ms, 
                                  site == "Canada" & task == "conv"),
                    family = 'binomial', 
                     control=glmerControl(optimizer="bobyqa",
                                             optCtrl=list(maxfun=2e4)))
anova(canada_circumstances_conv_base, canada_circumstances_conv, test = 'lrt') # significant effect of q_kind label, chisq = 94.73

## pairwise comparisons
emmeans::emmeans(canada_circumstances_conv, pairwise~q_kind_label, adjust = "none")
## significant difference between everyone else and rule at school
## significant difference between faraway country and rule at school
```
...
1.  Signficant effect of circumstance
2.  Moral and conventional: Everyone else and rule at school are significantly more permissible than faraway country

```{r}
## Moral
## singular fit with item level intercept
india_circumstances_moral_base <- glmer(answer ~ age.c + (1|subid),
                    data = subset(acceptability.ms, 
                                  site == "India" & task == "moral"),
                    family = 'binomial', 
                     control=glmerControl(optimizer="bobyqa",
                                             optCtrl=list(maxfun=2e4)))
## add q_kind_label
india_circumstances_moral <- glmer(answer ~ q_kind_label + age.c + (1|subid),
                    data = subset(acceptability.ms, 
                                  site == "India" & task == "moral"),
                    family = 'binomial', 
                     control=glmerControl(optimizer="bobyqa",
                                             optCtrl=list(maxfun=2e4)))
anova(india_circumstances_moral_base, india_circumstances_moral, test = 'lrt') # significant effect of q_kind label, chisq = 28.173

## pairwise comparisons
emmeans::emmeans(india_circumstances_moral, pairwise~q_kind_label, adjust = "none")
## significant difference between everyone else and faraway country
## significant difference between faraway country and rule at school

## conventional
## singular fit with item level intercept
india_circumstances_conv_base <- glmer(answer ~ age.c + (1|subid),
                    data = subset(acceptability.ms, 
                                  site == "India" & task == "conv"),
                    family = 'binomial', 
                     control=glmerControl(optimizer="bobyqa",
                                             optCtrl=list(maxfun=2e4)))
## add q_kind_label
india_circumstances_conv <- glmer(answer ~ q_kind_label + age.c + (1|subid),
                    data = subset(acceptability.ms, 
                                  site == "India" & task == "conv"),
                    family = 'binomial', 
                     control=glmerControl(optimizer="bobyqa",
                                             optCtrl=list(maxfun=2e4)))
anova(india_circumstances_conv_base, india_circumstances_conv, test = 'lrt') # significant effect of q_kind label, chisq = 94.76

## pairwise comparisons
emmeans::emmeans(india_circumstances_conv, pairwise~q_kind_label, adjust = "none")
## significant difference between everyone else and faraway country
## significant difference between faraway country and rule at schoo
## significant different difference between faraway country and rule at school
```

#### Iran
1.  Significant effect of circumstances
2.  Moral: No rule at school significantly higher than everyone else and faraway country
3.  Conventional: No rule at school is significantly more permissible than everyone else and faraway country

```{r}
## Moral
## singular fit with item level intercept
iran_circumstances_moral_base <- glmer(answer ~ age.c + (1|subid),
                    data = subset(acceptability.ms, 
                                  site == "Iran" & task == "moral"),
                    family = 'binomial', 
                     control=glmerControl(optimizer="bobyqa",
                                             optCtrl=list(maxfun=2e4)))
## add q_kind_label
iran_circumstances_moral <- glmer(answer ~ q_kind_label + age.c + (1|subid),
                    data = subset(acceptability.ms, 
                                  site == "Iran" & task == "moral"),
                    family = 'binomial', 
                     control=glmerControl(optimizer="bobyqa",
                                             optCtrl=list(maxfun=2e4)))
anova(iran_circumstances_moral_base, iran_circumstances_moral, test = 'lrt') # significant effect of q_kind label, chisq = 39.056

## pairwise comparisons
emmeans::emmeans(iran_circumstances_moral, pairwise~q_kind_label, adjust = "none")
## significant difference between everyone else and rule at school
## significant difference between faraway country and rule at school

## conventional
## singular fit with item level intercept
iran_circumstances_conv_base <- glmer(answer ~ age.c + (1|subid),
                    data = subset(acceptability.ms, 
                                  site == "Iran" & task == "conv"),
                    family = 'binomial', 
                     control=glmerControl(optimizer="bobyqa",
                                             optCtrl=list(maxfun=2e4)))
## add q_kind_label
iran_circumstances_conv <- glmer(answer ~ q_kind_label + age.c + (1|subid),
                    data = subset(acceptability.ms, 
                                  site == "Iran" & task == "conv"),
                    family = 'binomial', 
                     control=glmerControl(optimizer="bobyqa",
                                             optCtrl=list(maxfun=2e4)))
anova(iran_circumstances_conv_base, iran_circumstances_conv, test = 'lrt') # significant effect of q_kind label, chisq = 21.899

## pairwise comparisons
emmeans::emmeans(iran_circumstances_conv, pairwise~q_kind_label, adjust = "none")
## significant difference between everyone else and rule at school
## significant difference between faraway country and rule at school
...
## Moral
## singular fit with item level intercept
korea_circumstances_moral_base <- glmer(answer ~ age.c + (1|subid),
                    data = subset(acceptability.ms, 
                                  site == "Korea" & task == "moral"),
                    family = 'binomial', 
                     control=glmerControl(optimizer="bobyqa",
                                             optCtrl=list(maxfun=2e4)))
## add q_kind_label
korea_circumstances_moral <- glmer(answer ~ q_kind_label + age.c + (1|subid),
                    data = subset(acceptability.ms, 
                                  site == "Korea" & task == "moral"),
                    family = 'binomial', 
                     control=glmerControl(optimizer="bobyqa",
                                             optCtrl=list(maxfun=2e4)))
anova(korea_circumstances_moral_base, korea_circumstances_moral, test = 'lrt') # significant effect of q_kind label, chisq = 17.209

## pairwise comparisons
emmeans::emmeans(korea_circumstances_moral, pairwise~q_kind_label, adjust = "none")
## significant difference between everyone else and faraway country
## marginal difference between everyone else and rule at school
## marginal difference between faraway country and rule at school

## conventional
## singular fit with item level intercept
korea_circumstances_conv_base <- glmer(answer ~ age.c + (1|subid),
                    data = subset(acceptability.ms, 
                                  site == "Korea" & task == "conv"),
                    family = 'binomial', 
                     control=glmerControl(optimizer="bobyqa",
                                             optCtrl=list(maxfun=2e4)))
## add q_kind_label
korea_circumstances_conv <- glmer(answer ~ q_kind_label + age.c + (1|subid),
                    data = subset(acceptability.ms, 
                                  site == "Korea" & task == "conv"),
                    family = 'binomial', 
                     control=glmerControl(optimizer="bobyqa",
                                             optCtrl=list(maxfun=2e4)))
anova(korea_circumstances_conv_base, korea_circumstances_conv, test = 'lrt') # significant effect of q_kind label, chisq = 54.377

## pairwise comparisons
emmeans::emmeans(korea_circumstances_conv, pairwise~q_kind_label, adjust = "none")
## significant difference between everyone else and farway country
## significant difference between everyone else and rule at school
## significant difference between faraway country and rule at school
